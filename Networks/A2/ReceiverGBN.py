...
while (True):
	...
	if(data != None):
		...
		if(next_pck_id == int(word)):
			debug[int(word)].append(time.time() - start_time)
			if(random.random() > RANDOM_DROP_PROB):
				debug[next_pck_id].append(False)           	
				retrans = 0
